...
function extractSchemaLabel(schema, limit) {
	...
	if (!label) {
		label = '<no id> ' + valueStrim(schema, limit);
	}
	return label;
}
...
function reportSuccess(out, ctx) {
	out.writeln(out.success('success: ') + ctx.label);
	out.writeln(out.success('  -->    ') + extractSchemaLabel(ctx.schema));
}

function reportFailed(out, ctx, indent) {
	out.writeln(out.error(ctx.label));
	out.writeln(out.error('   !=  ') + extractSchemaLabel(ctx.schema));
...
function reportBulk(out, failed, passed, indent) {
	...
	if (failed.length > 0) {
		failed.some(function (ctx) {
			reportFailed(out, ctx, indent);
		});

		out.writeln('');
		out.writeln('tv4 ' + out.warning('validated ' + passed.length) + ', ' + out.error('failed ' + failed.length) + ' of ' + out.accent(total + ' ' + pluralise('values', total)));
		out.writeln('');
	}
	else if (total === 0) {
		out.writeln('');
		out.writeln('tv4 ' + out.warning('zero files'));
		out.writeln('');
	}
	else {
		out.writeln('');
		out.writeln('tv4 ' + out.success('validated ' + passed.length) + ' of ' + out.success(total + ' ' + pluralise('values', total)) + '\n');
		out.writeln('');
	}
