...
const isURL = (uri) => (/^https?:/v.test(uri) || uri.startsWith('file:'));

const headExp = /^(?<head>\w+):/v;
...
const getRunner = (tv4, loader, out, style) => {
	const getContext = (options) => {
		const context = {
			tv4: (options.fresh ? tv4.freshApi() : tv4),
			options: {},
		};
	...
	const tweakURI = (string_) => string_.split(/:\/\//v).map((string_) => string_.replace(/\//gv, () => repAccent)).join(repProto);

	const finaliseTask = (error, job, callback) => {
		job.success &&= (!job.error && job.failed.length === 0);
	...
	const recursiveTest = (job, object, callback) => {
		...
		if (object.result.missing.includes('')) {
			job.failed.push(object);
