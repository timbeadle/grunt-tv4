...
function getRunner(tv4, loader, out, style) {
  function getContext(options) {
    ...
    context.validate = function(objects, callback) {
      ...
      loadSchemaList(job, job.context.tv4.getMissingUris(), function(err) {
        ...
        forAsync(
          job.objects,
          function(object, index, callback) {
  ...
  function loadSchemaList(job, uris, callback) {
    ...
    var sweep = function() {
      ...
      forAsync(
        uris,
        function(uri, i, callback) {
          if (!uri) {
            out.writeln(
              '> ' + style.error('cannot load') + ' "' + tweakURI(uri) + '"'
            );
            callback();
          }
  ...
  function recursiveTest(job, object, callback) {
    ...
    if (object.result.missing.length === 0) {
      ...
      out.writeln(
        '> ' + style.success('pass') + ' | ' + tweakURI(object.label)
      );
      return callback();
    }
    ...
    if (
      !object.result.missing.every(function(value) {
        return value !== '';
      })
    ) {
      job.failed.push(object);
      out.writeln(
        '> ' +
          style.error('empty missing-schema url detected') +
          ' (this likely casued by a bad fragment pointer)'
      );
      return callback();
    }
  ...
  function doValidateObject(job, object, callback) {
    ...
    switch (t) {
      ...
      case 'string':
        ...
        if (schema) {
          out.writeln(
            '> ' + style.plain('have') + ' : ' + tweakURI(object.root)
          );
        ...
        out.writeln(
          '> ' + style.accent('root') + ' > ' + tweakURI(object.root)
        );
